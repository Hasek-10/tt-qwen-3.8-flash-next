...
//
// Final mixer stage 3a reader, one core: the D gathered fp32 partial row tiles [D, 1, 1, 32 T] (pages d * T + t) ->
// T stacked fp32 tiles in CB 0: tile t row d = partial d's row 0 of column tile t (columns 0-15 = face 0 row d,
...
// Compile-time args: 0 T, 1 D, then TensorAccessorArgs(partials), (zero tile).  Runtime args: 0 partials addr,
// 1 zero-tile addr.
#include <cstdint>
...
void kernel_main() {
    constexpr auto a_p = TensorAccessorArgs<2>();
    ...
    for (uint32_t t = 0; t < T; ++t) {
        for (uint32_t d = 0; d < D; ++d) {
            ...
            noc.async_read(partials, st, ROW_BYTES, {.page_id = d * T + t, .offset_bytes = 0}, {.offset_bytes = tile});
            noc.async_read(partials, st, ROW_BYTES, {.page_id = d * T + t, .offset_bytes = FACE_BYTES}, {.offset_bytes = tile + FACE_BYTES});
        }
    }
