...
//
// index_tail, rope core reader: the -1 scalar tile, the query cos/sin tiles, then the two hand-offs from the norm
// core: the query's RoPE tiles once, and per lane the pooled key's RoPE tiles (after signalling room for them) with
// block-start cos/sin tiles whose row 0 is the lane's row.
// Compile-time args: TensorAccessorArgs cos, sin, block_cos, block_sin.
// Runtime args: 0 cos, 1 sin, 2 block_cos, 3 block_sin addresses, 4 rows, 5 peer x, 6 peer y.
...
void kernel_main() {
    ...
    for (uint32_t lane = 0; lane < rows; ++lane) {
        ...
        noc_semaphore_wait_min(sem_k, lane + 1);
