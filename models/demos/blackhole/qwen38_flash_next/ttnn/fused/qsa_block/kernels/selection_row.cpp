...
// columns): sparse[i] = (((ids[i >> 2] << 2) + offsets[i]) & keep[i]) | fill[i] for i < 2048, (sentinel[i - 2048] &
// keep[i]) | fill[i] after, per row.  uint32 ROW_MAJOR rows: ids [rows, 512], offsets [1, 2048], sentinel [1, 32],
// keep / fill / out [rows, 2080].  CB 0: scratch (one 8 KB page).
// Compile-time args: TensorAccessorArgs ids, offsets, sentinel, keep, fill, out.
// Runtime args: 0-5 those addresses, 6 rows, 7 slice.
