...
// does not own are never read, so the fill the composed chain needs is gone.  The zero seed is the NoC's zero
// write from the firmware's zero block (no DRAM traffic: 80 cores reading one DRAM tile page cost 30 us).
...
void kernel_main() {
    ...
    noc.async_read(shared, CoreLocalMem<uint32_t>(shared_tile.get_write_ptr()), tile_bytes, {.page_id = tile_col}, {});
    ...
    const uint32_t act_base = act.get_write_ptr();
    ...
    for (uint32_t j = 0; j < rows; ++j) {
        noc.async_read(indices, CoreLocalMem<uint32_t>(idx_base + j * route_pitch), route_bytes, {.page_id = j}, {});
        noc.async_read(scores, CoreLocalMem<uint32_t>(sc_base + j * route_pitch), route_bytes, {.page_id = j}, {});
    }
    ...
    // deepseek_moe_fast_reduce_nc_fused_reader.cpp: tile k, column 0, row j = score[j][k] when the expert is on this
    // device's axis, else bf16 +0.0; rows past the tokens +0.0; the other columns are never read (COL broadcast)
    ...
    volatile tt_l1_ptr uint16_t* stile = reinterpret_cast<volatile tt_l1_ptr uint16_t*>(score_tiles.get_write_ptr());
    uint32_t owned[32];
    for (uint32_t j = 0; j < 32; ++j) {
        ...
        for (uint32_t k = 0; k < top_k; ++k) {
            uint16_t value = 0;
            if (j < rows && owner_u16[idx_u16[j * 32 + k]] != 0) {
                value = sc_u16[j * 32 + k];
                mask |= 1u << k;
            }
            stile[k * 1024 + col0] = value;
        }
        owned[j] = mask;
    }
    ...
    const uint32_t stage_base = (stage.get_write_ptr() + fragment_bytes - 1) & ~(fragment_bytes - 1);
    uint32_t n = 0;
    for (uint32_t j = 0; j < rows; ++j) {
        for (uint32_t k = 0; k < top_k; ++k) {
            if (owned[j] & (1u << k)) {
    ...
    for (uint32_t j = 0; j < rows; ++j) {
        for (uint32_t k = 0; k < top_k; ++k) {
            if (owned[j] & (1u << k)) {
                ...
                const uint32_t dst = act_base + k * tile_bytes + (j >> 4) * 2 * face_bytes + (j & 15) * face_row_bytes;
