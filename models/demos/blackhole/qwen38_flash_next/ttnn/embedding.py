...
class Qwen38TTNNLMHead:
    ...
    def sampling_candidate_rows(
        self, logits: Qwen38ShardedLogits, constants: Qwen38TTNNSamplingCandidateConstants, *, into
    ) -> None:
        """:meth:`sampling_candidates` for ``rows`` logit rows (1 .. 32): the MTP v2 sampled verify's candidate rows,
        landed in ``into`` (replicated FP32 ROW_MAJOR ``[1,1,rows,2 * TP_SIZE * k]``; row j is the candidate row of
        verify row j in the single-row layout).  The same ops with a row axis: the per-shard ``ttnn.topk`` over the
        rows, the SFPU widenings, the rebase, the pack, one all-gather and one landing copy; no id enters an FPU or
        reduce stage.  The greedy resolve of the same logits is untouched.
        """

        rows = self._validate_logits(logits)
        if self.collective_topology != ttnn.Topology.Linear:
            raise RuntimeError("sampling candidate rows require Linear topology")
        constants.validate(self.mesh_contract)
        k = SAMPLING_CANDIDATES_PER_DEVICE
        landing_shape = (1, 1, rows, 2 * TP_SIZE * k)

        def require(tensor, shape, dtype, layout, label) -> None:
            if _shape(tensor) != shape or tensor.dtype != dtype or tensor.layout != layout:
                raise RuntimeError(f"{label} must be {dtype} {layout} {shape}, got {_metadata(tensor)}")

        require(into, landing_shape, ttnn.float32, ttnn.ROW_MAJOR_LAYOUT, "sampling candidate rows landing")
        local_values, local_ids = ttnn.topk(logits.tensor, k, dim=-1, largest=True, sorted=True)
        ...
        row = ttnn.all_gather(pack, dim=3, cluster_axis=TP_AXIS, memory_config=ttnn.DRAM_MEMORY_CONFIG)
        _deallocate(pack)
        require(row, landing_shape, ttnn.float32, ttnn.ROW_MAJOR_LAYOUT, "sampling candidate rows")
        self.mesh_contract.validate_tensor(row, placement=TensorPlacement.REPLICATED)
        ttnn.copy(row, into)
        _deallocate(row)
