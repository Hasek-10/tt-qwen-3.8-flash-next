...
(the `--mtp` arm a greedy request drafts at; absent, a request with tools takes the largest arm, the rest the default;
a sampled request takes the 1-row loop and ignores it), `seed`,
